...
#endif
	sendto(server_socket, buf->start, buf->len, 0, (struct sockaddr *)&buf->peer, sizeof(buf->peer));
...
/*					if (!sc->throttle && !sc->needclose && !sc->closing) { */
		...
		if (server_socket>max) max=server_socket;
		FD_SET(control_fd,&readfds);
		if (control_fd>max) max=control_fd;
		tv.tv_sec=1;
		tv.tv_usec=0;
		schedule_unlock();
