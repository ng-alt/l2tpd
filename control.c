...
int control_finish(struct tunnel *t, struct call *c) {
	...
	if (debug_state)
		log(LOG_DEBUG, "%s: message type is %s(%d).  Tunnel is %d, call is %d.\n",__FUNCTION__,
...
/*				c->ourrws = c->lac->call_rws;
	...
	case SCCRP:
		...
		if (t->chal_them.state) {
			...
			if (memcmp(t->chal_them.reply, t->chal_them.response, MD_SIG_SIZE)) {
				set_error(c,VENDOR_ERROR, "Invalid challenge authentication");
		...
		log(LOG_LOG,
			...
			__FUNCTION__, IPADDY(t->peer.sin_addr),ntohs(t->peer.sin_port),t->ourtid, t->tid);
		if (t->lac) {
	...
	case SCCCN:
		...
		log(LOG_LOG,
			...
			IPADDY(t->peer.sin_addr),ntohs(t->peer.sin_port),t->ourtid, t->tid, t->lns->entname);
		/* Schedule a HELLO */
		tv.tv_sec = HELLO_DELAY;
		tv.tv_usec = 0;
#ifdef DEBUG_HELLO
	...
	case StopCCN:
		...
		log(LOG_LOG,
			...
			__FUNCTION__, IPADDY(t->peer.sin_addr),ntohs(t->peer.sin_port),t->self->errormsg, t->ourtid, t->tid);
		c->needclose = 0;
		c->closing = -1;
		break;
	case ICRQ:
...
/*		p->ourrws = p->lns->call_rws;
		...
		while(z) {
			if (z->cid == p->cid) {
				...
				log(LOG_DEBUG, "%s: Peer requested call %d twice, ignoring second one.\n",__FUNCTION__,p->cid);
				p->needclose = 0;
				p->closing = -1;
				return 0;
			}
...
/*		if (c->ourrws >= 0)
	...
	case ICCN:
		if (c==t->self) {
			log(LOG_DEBUG,
			"%s: Peer attempted ICCN on the actual tunnel, not the call",__FUNCTION__);
			return -EINVAL;
		}
		if (c->txspeed<1) {
			log(LOG_DEBUG,
		...
		po=add_opt(po, "passive");
		po=add_opt(po, "-detach");
		po=add_opt(po, "%s:%s", c->lns->localaddr ? ip1 : "", 
							    ip2);
		if (c->lns->authself) {
			if (c->lns->pap_refuse) 
				po=add_opt(po, "refuse-pap");
...
inline int check_control(const struct buffer *buf, struct tunnel *t, struct call *c) {
	...
	if (h->Ns != t->cSr) {
		...
		if (h->Ns < t->cSr) {
			...
			*/
#ifdef DEBUG_ZLB
			if (DEBUG) log(LOG_DEBUG, "%s: Sending an updated ZLB in reponse\n",__FUNCTION__);
#endif
			zlb = new_outgoing(t);
			control_zlb(zlb,t,c);
			udp_xmit(zlb);
			toss(zlb);
		} else if (!t->cSr && (t->tid == -1)) {
...
inline int expand_payload(struct buffer *buf, struct tunnel *t, struct call *c) {
	 ...
	 */
	if (!PLBIT(h->ver)) ehlen += 2;	/* Should have length information */
	if (!PFBIT(h->ver)) ehlen += 4; /* Should have Ns and Nr too */
	if (!PSBIT(h->ver)) ehlen += 4;	/* Offset information */
	if (ehlen) {
		/*
...
void send_zlb(void *data) 
{
	...
	c->pSs--;	/* We don't increment on ZLB's */
	c->zlb_xmit=NULL;
#ifdef DEBUG_ZLB
...
inline int write_packet(struct buffer *buf, struct tunnel *t, struct call *c, int convert) {
	...
	if (c->fd < 0) {
		if (DEBUG) log(LOG_DEBUG, "%s: tty is not open yet.\n",__FUNCTION__);
		return -EIO;
	}
	/*
	 * Skip over header 
