...
void call_close(struct call *c)
{
	...
	if  (c == c->container->self) {
		...
		*/
		...
		if (c->result < 0 ) c->result = RESULT_CLEAR;
		...
		*/
		...
		log(LOG_LOG,
			...
			c->container->tid,
			IPADDY(c->container->peer.sin_addr),ntohs(c->container->peer.sin_port),c->errormsg);
	} else {
		/*
		* Just close a call
		*/
	...
	if (c->container) {
		...
		if (p) {
			if (p == c) {
				...
				while(p->next && (p->next != c)) p=p->next;
				if (p->next) {
					p->next = c->next;
					c->container->count--;
				}
