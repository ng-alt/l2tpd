...
void init_addr() {
	int x;
	for (x=0;x<ADDR_HASH_SIZE;x++) uaddr[x]=NULL;
}
		...
		if (ipr->sense == SENSE_ALLOW)
			for (x=ntohl(ipr->start); x<=ntohl(ipr->end); x++) {
				...
				if (!ip_used(x)) {
					...
					ipr2 = ipr->next;
					while(ipr2) {
						if ((x >= ntohl(ipr2->start)) && (x <= ntohl(ipr2->end)))
							status=ipr2->sense;
						ipr2=ipr2->next;
					}
					y=htonl(x);
					if (status == SENSE_ALLOW) return y;
				}
			}; 
		ipr=ipr->next;
	}
...
int get_secret(char *us, char *them, char *secret, int size)
{
	...
	while(!feof(f)) {
		...
		if (feof(f)) break;
		/* Strip comments */
		for (t=buf; *t; t++) *t = ((*t == '#') || (*t == ';')) ? 0 : *t;
		/* Strip trailing whitespace */
		for (t=buf+strlen(buf)-1; (t>=buf) && (*t<33); t--) *t=0;
		if (!strlen(buf)) continue; 	/* Empty line */
		u=buf;
		while(*u && (*u < 33)) u++;
		/* us */
		if (!*u) {
			log(LOG_WARN, "%s: Invalid authentication info (no us), line %d\n",__FUNCTION__,num); 			
			continue;
		}
		t=u;
		while(*t>32) t++;
		*(t++)=0;
		...
		/* them */
		if (!*t) {
			log(LOG_WARN, "%s: Invalid authentication info (nothem), line %d\n",__FUNCTION__,num); 			
			continue;
		}
		s=t;
		while(*s >33) s++;
		*(s++)=0;
		while (*s && (*s < 33)) s++;
		if (!*s) {
			log(LOG_WARN, "%s: Invalid authentication info (no secret), line %d\n",__FUNCTION__,num); 			continue;
		}
		if ((!strcasecmp(u,us) || !strcasecmp(u,"*")) &&
		    (!strcasecmp(t,them) || !strcasecmp(t,"*"))) {
#ifdef DEBUG_AUTH
				log(LOG_DEBUG, "%s: we are '%s', they are '%s', secret is '%s'\n",__FUNCTION__,u,t,s); 
#endif
...
#ifdef DEBUG_AAA
					log(LOG_DEBUG, "get_lns: Rule %s to %s, sense %s matched %s\n",
...
void encrypt_avp(struct buffer *buf, _u16 len, struct tunnel *t)
{
	/* Encrypts an AVP of len, at data.  We assume there
	   ...
	   an avpsend routine */
	struct avp_hdr *new_hdr = (struct avp_hdr *)(buf->start + buf->len - len);
	struct avp_hdr *old_hdr = (struct avp_hdr *)(buf->start + buf->len - len+2);
	...
	length = ((len - sizeof(struct avp_hdr) + 1) / 16 + 1) * 16 + sizeof(struct avp_hdr);
	...
	MD5Update(&t->chal_them.md5, t->chal_them.secret, strlen(t->chal_them.secret));
...
int decrypt_avp(char *buf, struct tunnel *t)
{
	...
	MD5Update(&t->chal_us.md5, t->chal_us.secret, strlen(t->chal_us.secret));
	...
	while(ptr < end) {
		 ...
		 */
		if (cnt == 0) {
			saved_segment_len = (end-ptr<MD_SIG_SIZE)?(end-ptr):MD_SIG_SIZE;
	...
	if (len > olen - 2) {
		log(LOG_DEBUG, "decrypt_avp: Decrypted length is too long (%d > %d)\n",len, olen - 2);
		return -EINVAL;
	}
