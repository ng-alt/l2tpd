...
int yesno(char *value)
{
		if (!strcasecmp(value,"yes") || !strcasecmp(value,"y") ||
			!strcasecmp(value,"true")) return 1;
		else if (!strcasecmp(value,"no") || !strcasecmp(value,"n") ||
			!strcasecmp(value,"false")) return 0;
		else return -1;
}
...
{
	if (atoi(value)< 1) {
		snprintf(filerr, sizeof(filerr), "rtimeout value must be at least 1\n");
		return -1;
	}
	switch(context & ~CONTEXT_DEFAULT) {
	case CONTEXT_LAC:
...
{
	if (atoi(value)< -1) {
		snprintf(filerr, sizeof(filerr), "receive window size must be at least -1\n");
		return -1;
	}
	switch(context & ~CONTEXT_DEFAULT) {
	case CONTEXT_LAC:
		...
		if (word[0]=='t') {
			...
			if (((struct lac *)item)->tun_rws<1) {
				snprintf(filerr, sizeof(filerr), "receive window size for tunnels must be at least 1\n");
				return -1;
			}
		}
		break;
...
{
	if (!strlen(value)) {
		snprintf(filerr, sizeof(filerr), "no filename specified for authentication\n");
		return -1;
	}
	switch(context & ~CONTEXT_DEFAULT) {
	case CONTEXT_GLOBAL:
...
int set_accesscontrol(char *word, char *value, int context, void *item)
{
	switch(context & ~CONTEXT_DEFAULT) {
	case CONTEXT_GLOBAL:
		if (set_boolean(word,value,&(((struct global *)item)->accesscontrol)))
			return -1;
		break;
	default:
		snprintf(filerr, sizeof(filerr),"'%s' not valid in this context\n",word);
...
int set_userspace(char *word, char *value, int context, void *item)
{
	switch(context & ~CONTEXT_DEFAULT) {
	case CONTEXT_GLOBAL:
		if (set_boolean(word,value,&(((struct global *)item)->forceuserspace)))
			return -1;
		break;
	default:
		snprintf(filerr, sizeof(filerr),"'%s' not valid in this context\n",word);
...
struct iprange *set_range(char *word, char *value, struct iprange *in)
{
	...
	if (c) {
		d=c+1;
		*c=0;
		while((c>=value) && (*c < 33)) *(c--)=0;
		while(*d && (*d < 33)) d++;
	}
	if (!strlen(value) || (c && !strlen(d))) {
		snprintf(filerr, sizeof(filerr), "format is '%s <host or ip> - <host or ip>'\n",word);
		return NULL;
	}
	...
	if (word[0]=='n') ipr->sense=SENSE_DENY; else ipr->sense=SENSE_ALLOW;
	p=in;
	if (p) {
...
int set_lns(char *word, char *value, int context, void *item)
{
	...
	case CONTEXT_LAC:
		...
		if (d) ipr->port=atoi(d); else ipr->port=UDP_LISTEN_PORT;
		break;
	default:
		snprintf(filerr, sizeof(filerr),"'%s' not valid in this context\n",word);
		return -1;
...
int parse_config(FILE *f)
{
	...
	while(!feof(f)) {
		...
		*s=0;
		...
		while((*s < 33) && *s) s++;	/* Skip over beginning white space */
		t=s+strlen(s);
		while((t >= s) && (*t < 33)) *(t--)=0;	/* Ditch trailing white space */
		...
		if (s[0] == '[') {
			...
			if (!strcasecmp(s,"global")) {
...
#ifdef DEBUG_FILE
				log(LOG_DEBUG, "parse_config: global context descriptor %s\n", d ? d : "");
#endif
				data=&gconfig;
				...
				if (def) {
					if (!deflns) {
						deflns=new_lns();
						strncpy(deflns->entname, "default", sizeof(deflns->entname));
					}
					data=deflns;
					continue;
				}
				...
				if (def) {
					if (!deflac) {
						deflac=new_lac();
						strncpy(deflac->entname, "default", sizeof(deflac->entname));
					}
					data=deflac;
					continue;
				}
				...
				log(LOG_WARN, "parse_config: line %d: unknown context '%s'\n",linenum,s);
				return -1;
			}
		} else {
			if (!context) {
			...
			for (kw=words;kw->keyword;kw++) {
				if (!strcasecmp(s,kw->keyword)) {
					if (kw->handler(s,t,context | def,data)) {
						log(LOG_WARN, "parse_config: line %d: %s",linenum,filerr);
						return -1;
					}
					break;
				}
