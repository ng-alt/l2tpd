...
{
	if (min)
		snprintf(c->errormsg, sizeof(c->errormsg),"%s: expected at least %d, got %d",
		field,expected,found);
	else
		snprintf(c->errormsg, sizeof(c->errormsg),"%s: expected %d, got %d",
		field,expected,found);
...
int message_type_avp(struct tunnel *t, struct call *c, void *data, int datalen)
{
	...
	if (datalen != 8) {
		if (DEBUG) log(LOG_DEBUG, "%s: wrong size (%d != 8)\n",__FUNCTION__,datalen);
		wrong_length(c,"Message Type",8,datalen,0);
		return -EINVAL;
	}
	if ((c->msgtype > MAX_MSG) || (!msgtypes[c->msgtype])) {
		if (DEBUG) log(LOG_DEBUG, "%s: unknown message type %d\n",__FUNCTION__,c->msgtype);
		return -EINVAL;
	}
if (debug_avp)
	if (DEBUG) log(LOG_DEBUG, "%s: message type %d (%s)\n",__FUNCTION__,c->msgtype, msgtypes[c->msgtype]);
#ifdef SANITY
	if (t->sanity) {
	/*
	 ...
	 */
		if ((c != t->self) && (c->msgtype < Hello) ) {
			if (DEBUG) log(LOG_DEBUG, "%s: attempting to negotiate tunnel inside a call!\n",__FUNCTION__);
			return -EINVAL;
		}
		...
		case SCCRQ: 
			if ((t->state != 0) && (t->state != SCCRQ)) {
				...
				if (DEBUG) log(LOG_DEBUG, "%s: attempting to negotiate SCCRQ with state != 0\n",__FUNCTION__);
				return -EINVAL;
			}
			break;
		case SCCRP:
			if (t->state != SCCRQ) {
				if (DEBUG) log(LOG_DEBUG, "%s: attempting to negotiate SCCRP with state != SCCRQ!\n",__FUNCTION__);
				return -EINVAL;
			}
			break;
		case SCCCN:
			if (t->state != SCCRP) {
				if (DEBUG) log(LOG_DEBUG, "%s: attempting to negotiate SCCCN with state != SCCRP!\n",__FUNCTION__);
				return -EINVAL;
			}
			break;
		case ICRQ:
			if (t->state != SCCCN) {
				if (DEBUG) log(LOG_DEBUG, "%s: attempting to negotiate ICRQ when state != SCCCN\n",__FUNCTION__);
				return -EINVAL;
			}
			if (c != t->self) {
				if (DEBUG) log(LOG_DEBUG, "%s: attempting to negotiate ICRQ on a call!\n",__FUNCTION__);
				return -EINVAL;
			}
			break;
		case ICRP:
			if (t->state != SCCCN) {
				if (DEBUG) log(LOG_DEBUG, "%s: attempting to negotiate ICRP on tunnel!=SCCCN\n",__FUNCTION__);
				return -EINVAL;
			}
			if (c->state != ICRQ) {
				if (DEBUG) log(LOG_DEBUG, "%s: attempting to negotiate ICRP when state != ICRQ\n",__FUNCTION__);
				return -EINVAL;
			}
			break;
		case ICCN:
			if (c->state != ICRP) {
				if (DEBUG) log(LOG_DEBUG, "%s: attempting to negotiate ICCN when state != ICRP\n",__FUNCTION__);
				return -EINVAL;
			}
			break;
		case SLI:
...
int hostname_avp(struct tunnel *t, struct call *c, void *data, int datalen) {
	...
	size = raw[0] & 0x0FFF;
	if (size > MAXSTRLEN-1) {
		if (DEBUG) log(LOG_DEBUG,"%s: truncating reported hostname (size is %d)\n",__FUNCTION__,size);
		size = MAXSTRLEN-1;
	}
...
int dialing_number_avp(struct tunnel *t, struct call *c, void *data, int datalen) {
	...
	size = raw[0] & 0x0FFF;
	if (size > MAXSTRLEN-1) {
		if (DEBUG) log(LOG_DEBUG,"%s: truncating reported dialing number (size is %d)\n",__FUNCTION__,size);
		size = MAXSTRLEN-1;
	}
...
int dialed_number_avp(struct tunnel *t, struct call *c, void *data, int datalen) {
	...
	size = raw[0] & 0x0FFF;
	if (size > MAXSTRLEN-1) {
		if (DEBUG) log(LOG_DEBUG,"%s: truncating reported dialed number (size is %d)\n",__FUNCTION__,size);
		size = MAXSTRLEN-1;
	}
...
int sub_address_avp(struct tunnel *t, struct call *c, void *data, int datalen) {
	...
	size = raw[0] & 0x0FFF;
	if (size > MAXSTRLEN-1) {
		if (DEBUG) log(LOG_DEBUG,"%s: truncating reported sub address (size is %d)\n",__FUNCTION__,size);
		size = MAXSTRLEN-1;
	}
...
int vendor_avp(struct tunnel *t, struct call *c, void *data, int datalen) {
	...
	size = raw[0] & 0x0FFF;
	if (size > MAXSTRLEN-1) {
		if (DEBUG) log(LOG_DEBUG,"%s: truncating reported vendor (size is %d)\n",__FUNCTION__,size);
		size = MAXSTRLEN-1;
	}
...
int call_serno_avp(struct tunnel *t, struct call *c, void *data, int datalen) {
	...
	if (t->sanity) {
		...
		if (datalen != 10) {
			...
			return -EINVAL;
#else
			log(LOG_DEBUG, "%s: peer is using old style serial number.  Will be invalid.\n",__FUNCTION__);
#endif			
...
if (debug_avp) {
	if (DEBUG)
		log(LOG_DEBUG,
			"%s: serial number is %d\n",__FUNCTION__,t->call_head->serno);
}
	return 0;		
}
...
if (debug_avp) {
	if (DEBUG)
		log(LOG_DEBUG,
			"%s: physical channel is %d\n",__FUNCTION__,t->call_head->physchan);
}
	return 0;		
}
...
int handle_avps(struct buffer *buf, struct tunnel *t, struct call *c) {
	...
	while (len>0) {
		...
		if (avp->attr > AVP_MAX) {
			if (AMBIT(avp->length)) {
				log(LOG_WARN, "%s:  dont know how to handle mandatory attribute %d.  Closing %s.\n"__FUNCTION__,avp->attr,(c != t->self) ? "call" : "tunnel");
				set_error(c,VENDOR_ERROR,"mandatory attribute %d cannot be handled",avp->attr);
				c->needclose = -1;
				return -EINVAL;
			} else {
				if (DEBUG) log(LOG_WARN, "%s:  dont know how to handle atribute %d.\n",__FUNCTION__, avp->attr);
				goto next;
			}
		}
		if (ALENGTH(avp->length) > len) {
			log(LOG_WARN, "%s: AVP received with length > remaining packet length!\n",__FUNCTION__);
		...
		if (avps[avp->attr].handler) {
			if (avps[avp->attr].handler(t, c, avp, ALENGTH(avp->length))) {
				if (AMBIT(avp->length)) {
					log(LOG_WARN, "%s: Bad exit status handling attribute %d (%s) on mandatory packet.\n",__FUNCTION__,avp->attr, avps[avp->attr].description);
					c->needclose=-1;
					return -EINVAL;
				} else {
					if (DEBUG) log(LOG_DEBUG,"%s: Bad exit status handling attribute %d (%s).\n",__FUNCTION__,avp->attr, avps[avp->attr].description);
				}
			}
		} else {
			if (AMBIT(avp->length)) {
				log(LOG_WARN, "%s:  No handler for mandatory attribute %d (%s).  Closing %s.\n",__FUNCTION__,avp->attr, avps[avp->attr].description,(c != t->self) ? "call" : "tunnel");
				set_error(c,VENDOR_ERROR,"No handler for attr %d (%s)\n",avp->attr,avps[avp->attr].description);
				return -EINVAL;
			} else {
				if (DEBUG) log(LOG_WARN, "%s:  no handler for atribute %d (%s).\n",__FUNCTION__, avp->attr, avps[avp->attr].description);
			}
		}
next:
		if (hidlen) {
